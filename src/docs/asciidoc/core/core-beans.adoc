...
with these classes directly (that is, programmatically) but rather with XML `ioc`
...
XSD, since it does not provide value over a regular `ioc` reference any more. Change
...
other object through the `ioc` or `parent` attribute.

Specifying the target bean through the `ioc` attribute of the `<ref/>` tag is the most
...
`ioc` attribute may be the same as the `id` attribute of the target bean or be the same
...
The p-namespace lets you use the `ioc` element's attributes (instead of nested
...
child elements of the `ioc` element in the XML. When relying upon classpath scanning for
...
on the `ioc` element, as the following example shows:
