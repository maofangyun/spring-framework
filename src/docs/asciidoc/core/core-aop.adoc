...
Spring AOP also supports an additional PCD named `ioc`. This PCD lets you limit
the matching of join points to a particular named Spring bean or to a set of named
Spring beans (when using wildcards). The `ioc` PCD has the following form:
...
conventions for your Spring beans, you can write a `ioc` PCD expression
to select them. As is the case with other pointcut designators, the `ioc` PCD can
be used with the `&&` (and), `||` (or), and `!` (negation) operators, too.

[NOTE]
====
The `ioc` PCD is supported only in Spring AOP and not in
...
The `ioc` PCD operates at the instance level (building on the Spring bean name
